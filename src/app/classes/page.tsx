...
import { useStore, type Class } from "@/lib/store";
...
export default function ClassesPage() {
  ...
  const handleDelete = (id: string) => {
    ...
    if (studentCount > 0) {
      return alert(`Cannot delete: ${studentCount} students are still assigned to this class.`);
    }
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {classes.length === 0 && (
          <div className="col-span-full py-20 bg-white rounded-2xl border border-dashed border-gray-200 text-center">
            ...
            <p className="text-gray-400 text-sm font-medium">No classes defined. Start by adding one.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b flex items-center justify-between bg-gray-50/50">
              <h2 className="text-lg font-bold text-gray-800">{editing ? "Edit Class" : "New Class"}</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Class Name</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Section / Room</label>
                <input 
                  type="text" 
                  value={form.section} 
