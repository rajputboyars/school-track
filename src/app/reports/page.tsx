...
export default function ReportsPage() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await getAttendanceReport(classFilter);

      setReportData(res.data); // expected aggregated data
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
    ...
    <div className="p-8 mx-auto">
      <header className="mb-8">
        ...
        <p className="text-gray-500 text-sm">
          Analyze attendance trends and performance
        </p>
      </header>
            ...
            {(["student", "class", "subject"] as const).map((v) => (
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all ${
                  view === v
                    ? "bg-white text-[#1e816a] shadow-sm"
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {reportData.map((r: any, i) => (
              <tr key={i}>
                ...
                <td className="px-6 py-4">{r.present}</td>
                <td className="px-6 py-4">
                  <PercentBar value={r.percent} />
                </td>
              </tr>
